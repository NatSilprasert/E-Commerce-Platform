...
const Navbar = () => {
    ...
    return (
        <nav className='fixed w-full z-40 top-0 bg-white border border-gray-300 px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw]'>
            <div className='flex items-center justify-between font-medium py-5'>
                ...
                <div className={`absolute h-screen top-0 right-0 bottom-0 overflow-hidden bg-white transition-all duration-300 ${visible ? 'w-full' : 'w-0'}`}>
                    <div className='flex flex-col text-gray-600'>
                        <div onClick={() => setVisible(false)} className='flex items-center gap-4 py-8 mx-3 cursor-pointer'>
                        ...
                        {token && <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border-b border-gray-300" to='/orders'>ORDERS</NavLink>}
                        ...
                        {token && <div onClick={() => (setVisible(false),logout())} className="py-2 pl-6 border-b border-gray-300">LOGOUT</div>}
                    </div> 
                </div>
